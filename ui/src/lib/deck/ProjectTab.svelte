  ...
  <div class="left">
    ...
    <div class="doc">
      ...
      <button class="more" aria-expanded={docOpen} on:click={() => (docOpen = !docOpen)}>
        {docOpen ? "Show less" : "More"}
      </button>
    </div>
    {:else if !events.length && !streaming}
      <div class="empty">The header writes the brief here.</div>
    {/if}
...
<style>
  ...
  .empty { font-size: 13px; color: var(--text-4); padding: 24px 2px; }
