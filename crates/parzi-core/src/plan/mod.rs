...
//!
//! Two grammars share this module. The legacy one (milestones, checklist
//! tasks, `[lane:x]` tags) is what today's projects on disk hold and is
//! parsed by `parse_plan`; the hub grammar (sprints, lanes, tasks with
//! `[repo:]`/`[scope:]`/`[after:]`, PLAN §3) lives in `v1` and round-trips.
...
pub use v1::{
    parse_plan_v1, render_plan, LanePlan, Plan, Sprint, Task, TaskId, PLAN_GRAMMAR_VERSION,
};
